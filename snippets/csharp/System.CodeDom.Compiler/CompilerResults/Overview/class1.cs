    ...
    class Class1
    {
        ...
        public static void DisplayCompilerResults(System.CodeDom.Compiler.CompilerResults cr)
        {
            ...
            if (cr.Errors.Count > 0)
            {
                ...
                {
                    Console.WriteLine($"{i}: {cr.Errors[i]}");
                }
            }
            else
            {
                // Display information about the compiler's exit code and the generated assembly.
                Console.WriteLine($"Compiler returned with result code: {cr.NativeCompilerReturnValue}");
                ...
                else
                {
                    ...
                    {
                        Console.WriteLine($"TempFile {i}: {(string)enu.Current}");
                    }
                }
            }
        }
