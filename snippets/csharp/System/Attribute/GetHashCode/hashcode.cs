...
public class AuthorsAttribute : Attribute
{
    ...
    public string AuthorName2
   {
    ...
    public override int GetHashCode() => _authorName1.GetHashCode() ^ _authorName2.GetHashCode();
}
...
class Example
{
    static void Main()
    {
        ...
        for (int ctr = 0; ctr < authorsInfo.Count; ctr++)
        {
            ...
            if (!listed[ctr])
            {
                Console.WriteLine($"{authorInfo.Item2.AuthorName1} and {authorInfo.Item2.AuthorName2}");
                listed[ctr] = true;
                Console.WriteLine($"   {authorInfo.Item1}");
                for (int ctr2 = ctr + 1; ctr2 < authorsInfo.Count; ctr2++)
                {
                    if (!listed[ctr2])
                        if (authorInfo.Item2.Equals(authorsInfo[ctr2].Item2))
                        {
                            Console.WriteLine($"   {authorsInfo[ctr2].Item1}");
                            listed[ctr2] = true;
                        }
                }
            }
