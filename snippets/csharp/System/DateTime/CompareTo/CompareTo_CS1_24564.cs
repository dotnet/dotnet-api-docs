...
public class DateTimeComparison
{
    ...
    public static void Main()
    {
        ...
        Console.WriteLine($"CompareTo method returns {(int)comparison}: {thisDate:d} is {comparison.ToString().ToLower()} than {thisDateLastYear:d}");

        // Compare today to next year
        comparison = (DateComparisonResult)thisDate.CompareTo(thisDateNextYear);
        Console.WriteLine($"CompareTo method returns {(int)comparison}: {thisDate:d} is {comparison.ToString().ToLower()} than {thisDateNextYear:d}");
    }
}
//
