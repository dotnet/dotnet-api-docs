...
public class FileTimes
{
    ...
    public static void Main()
    {
        ...
        {
            Console.WriteLine($"Unable to access {winDir}.");
        }
        else
        {
            ...
            if (GetFileTime(hFile, out creationTime, out accessTime, out writeTime))
            {
                ...
                Console.WriteLine($"File {winDir} Retrieved Using the Windows API:");
                Console.WriteLine($"   Created:     {DateTimeOffset.FromFileTime(fileCreationTime).ToString():d}");
                Console.WriteLine($"   Last Access: {DateTimeOffset.FromFileTime(fileAccessTime).ToString():d}");
                Console.WriteLine($"   Last Write:  {DateTimeOffset.FromFileTime(fileWriteTime).ToString():d}");
                Console.WriteLine();
            }
        }

        // Get date and time, convert to file time, then convert back
        FileInfo fileInfo = new(winDir);
        ...
        Console.WriteLine($"File {winDir} Retrieved Using a FileInfo Object:");
        Console.WriteLine($"   Created:     {DateTimeOffset.FromFileTime(ftCreationTime).ToString():d}");
        Console.WriteLine($"   Last Access: {DateTimeOffset.FromFileTime(ftAccessTime).ToString():d}");
        Console.WriteLine($"   Last Write:  {DateTimeOffset.FromFileTime(ftWriteTime).ToString():d}");
    }
}
