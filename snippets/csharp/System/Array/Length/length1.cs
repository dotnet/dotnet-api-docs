...
public class Example
{
    ...
    {
        Console.WriteLine($"Length of Array:      {arr.Length,3}");
        Console.WriteLine($"Number of Dimensions: {arr.Rank,3}");
        // For multidimensional arrays, show number of elements in each dimension.
        if (arr.Rank > 1)
        {
            for (int dimension = 1; dimension <= arr.Rank; dimension++)
                Console.WriteLine($"   Dimension {dimension}: {arr.GetUpperBound(dimension - 1) + 1,3}");
        }
        Console.WriteLine();
    }
}
