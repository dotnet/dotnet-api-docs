...
public static class DisplayChars
{
            ...
            switch (args.Length)
            {
                ...
                case 3:
                    if (!uint.TryParse(args[0], NumberStyles.HexNumber, null, out rangeStart))
                        throw new ArgumentException($"{args[0]} is not a valid hexadecimal number.");

                    if (!uint.TryParse(args[1], NumberStyles.HexNumber, null, out rangeEnd))
                        throw new ArgumentException($"{args[1]} is not a valid hexadecimal number.");

                    bool.TryParse(args[2], out setOutputEncodingToUnicode);
                    break;
                default:
                    Console.WriteLine($"Usage: {Environment.GetCommandLineArgs()[0]} <{"startingCodePointInHex"}> <{"endingCodePointInHex"}> [{"<setOutputEncodingToUnicode?{true|false, default:false}>"}]");
                    return;
            }
    ...
    public static void DisplayRange(uint start, uint end)
    {
        ...
        if ((start < surrogateStart & end > surrogateStart) || (start >= surrogateStart & start <= surrogateEnd))
            throw new ArgumentException($"0x{start:X5}-0x{end:X5} includes the surrogate pair range 0x{surrogateStart:X5}-0x{surrogateEnd:X5}");
    ...
    private static uint RoundUpToMultipleOf(uint b, uint u) => RoundDownToMultipleOf(b, u) + b;

    private static uint RoundDownToMultipleOf(uint b, uint u) => u - (u % b);
}
