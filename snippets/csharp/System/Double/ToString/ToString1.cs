...
public class Class1
{
    ...
    private static void CallToStringWithFormatString()
    {
        ...
        foreach (double number in numbers)
        {
            ...
            {
                Console.WriteLine($"   {specifier + ":",-22} {number.ToString(specifier)}");
                // Add precision specifiers from 0 to 3.
                if (specifier.Length == 1 & !specifier.Equals("R"))
                {
                    for (int precision = 0; precision <= 3; precision++)
                    {
                        string pSpecifier = $"{specifier}{precision}";
                        Console.WriteLine($"   {pSpecifier + ":",-22} {number.ToString(pSpecifier)}");
                    }
                    Console.WriteLine();
                }
            }
