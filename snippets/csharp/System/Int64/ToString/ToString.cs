...
public class LongToString
{
    ...
    private static void CallToStringWithSpecificSpecifiers()
    {
        ...
        specifier = "P";
        Console.WriteLine($"{specifier}: {(value / 100000.0).ToString(specifier)}");
        // Displays:    P: -16.33 %
        specifier = "X";
        Console.WriteLine($"{specifier}: {value.ToString(specifier)}");
        ...
        specifier = "#,#.00#;(#,#.00#)";
        Console.WriteLine($"{specifier}: {(value * -1).ToString(specifier)}");
    ...
    private static void CallToStringWithSpecifiersAndCultures()
    {
        ...
        foreach (string specifier in specifiers)
        {
            foreach (CultureInfo culture in cultures)
            {
                ...
                Console.WriteLine($"{specifier} format using {culture.Name} culture: {positiveNumber.ToString(specifier, culture),16} {negativeNumber.ToString(specifier, culture),16}");
            }
            Console.WriteLine();
        }
