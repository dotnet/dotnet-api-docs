...
public class ParseSample
{
    ...
    private static void ParseToInt16(string value, NumberStyles style)
    {
        try
        {
            short number = short.Parse(value, style);
            Console.WriteLine($"Converted '{value}' to {number}.");
        }
        catch (FormatException)
        {
            Console.WriteLine($"Unable to parse '{value}' with style {style.ToString()}.");
        }
        catch (OverflowException)
        {
            Console.WriteLine($"'{value}' is out of range of the Int16 type.");
        }
    }
}
