...
- Use the [Exceptions report](http://vivodina/) to determine which exceptions a specific member throws. The report also indicates whether the exceptions thrown by a member are documented or undocumented, and it allows you to tag the exceptions that you've documented. However, you should investigate each exception that is identified in the report as undocumented by examining the source code. In gathering exception information, the backend application examines nearly the complete call stack for a method. It sometimes has difficulty eliminating exceptions that are thrown farther in the call stack by a method that is passed a set of default parameters. (This might happen, for example, if a parameterless constructor passes a string argument to another constructor overload, which throws an exception if the string argument passed to it is null. In this case, the string argument can never be null, but the exception can nevertheless appear in the Exceptions report.)
