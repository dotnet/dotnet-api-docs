...
When a class explicitly defines multiple overloads, the first overload topic appears in DxEditor with the **Overload:** preface. This preface is required by the build tools; do not change or delete it. You must write the general summary broadly enough to apply to all the individual overloads. Follow the summary wording guidelines for the particular kind of member you are documenting (for more information, see the relevant subsection under <a href="efd07549-bbec-49ab-94a5-6b88f06f0a2e.htm">Summary Section</a>), and use text that is programming language-neutral. For an example, see the table in the next section.
...
Provide a separate summary for each individual overload topic in DxEditor. Use wording that reflects that overload's parameters and distinguishes that overload from the general summary and from the summaries of other overloads. The summary should provide enough information to help users select the overload that they'd like to call.
